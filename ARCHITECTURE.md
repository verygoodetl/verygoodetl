...
4. The runtime closes the stage's downstream edges once it returns, whether or not `Finish` ever ran: for a `Source`, that's when `Run` returns; for a `Processor` or `Sink`, that's when `Finish` returns on the success path described in point 3, or immediately — with `Finish` skipped entirely — if that stage's own inputs failed, its own processing failed, or a single check of the context right before `Finish` would run finds it already canceled.

If any stage fails, the pipeline context is canceled before that stage's downstream edges are closed. Downstream stages therefore must not interpret an upstream failure as a successful end-of-stream and run `Finish` on partial data — instead, a stage whose inputs failed, or whose context is observed canceled at that one check, skips `Finish` entirely, and may have `Abort` called on it once instead (see `Aborter`). That check is a single point-in-time read of `ctx.Err()`, not an atomic guard: cancellation arriving in the narrow window after it passes still lets `Finish` run, so a stage cannot rely on "canceled" and "`Finish` was skipped" as equivalent — only that skipping implies cancellation was already visible, not the reverse. A stage should not treat `Finish` as its only chance to run required cleanup or commit logic; that logic must be reachable from `Abort` too, since a failed or canceled run may never reach `Finish` at all.
...
Once `Run` has actually started a pipeline's stages, it always waits for every one of them to finish unwinding, then returns a single error — never the bare fact that ctx was canceled. `Run` does not aggregate every failing node's error; when multiple stages fail, it records one and discards the rest, preferring the first non-cancellation error over a `context.Canceled`/`context.DeadlineExceeded` cascade so the reported error reflects the actual root cause rather than whichever cancellation-triggered failure happened to land first. When a stage's own callback (`Process`, `Consume`, `Finish`) respects its context and returns an error once canceled, that error is a candidate for the recorded result, typically `ctx.Err()`. But a `Processor` or `Sink` can also be canceled while no callback of its own is running at all — for instance, while the runtime is blocking on its behalf waiting for the next input batch — and in that case there is no stage-returned error to record, so the runtime synthesizes `ctx.Err()` itself as that node's result. But if every stage completes without an error, `Run` returns `nil` even when ctx was also canceled: the pipeline's work finished before the cancellation could have had any effect on it, so nothing was actually canceled.
